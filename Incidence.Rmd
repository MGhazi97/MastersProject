...
incidenceAges2 <- leukemiaIncidenceDesc %>% filter(!is.na(Count)) %>% dplyr::select(Age.Group, Count) %>% group_by(Age.Group) %>% count(Age.Group, wt=Count) %>% rename("Count" = "n"); incidenceAges2

n <- sum(incidenceAges2$Count)

incidenceAges2 <- mutate(incidenceAges2, Percent = 100*Count/n); incidenceAges2

ggplot(incidenceAges2, aes(x = Age.Group, y = Count)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Count), hjust = -0.5)  + coord_flip() +
  scale_y_continuous(labels = function(x) format(x, scientific=FALSE)) +
  ylab("Count") +
...
```{r}
incidenceSex2 <- leukemiaIncidenceDesc %>% filter(!is.na(Count)) %>% dplyr::select(Sex, Count) %>% group_by(Sex) %>% count(Sex, wt=Count) %>% rename("Count" = "n"); incidenceSex2

n <- sum(incidenceSex2$Count)

incidenceSex2 <- mutate(incidenceSex2, Percent = 100*Count/n); incidenceSex2

ggplot(incidenceSex2, aes(Sex)) + geom_bar(aes(weight = Count)) + ylab("Count") + xlab("Sex") + ggtitle("Incidences by Sex Group")
```
Doesnt exist
#```{r}
...
```{r}
incidenceYOI2 <- leukemiaIncidenceDesc %>% filter(!is.na(Count)) %>% filter(Year.of.diagnosis != "1975-2019") %>% dplyr::select(Year.of.diagnosis, Count) %>% group_by(Year.of.diagnosis) %>% count(Year.of.diagnosis, wt=Count) %>% rename("Count" = "n"); incidenceYOI2

n <- sum(incidenceYOI2$Count)

incidenceYOI2 <- mutate(incidenceYOI2, Percent = 100*Count/n); incidenceYOI2

ggplot(incidenceYOI2, aes(Year.of.diagnosis)) + geom_bar(aes(weight = Count)) + ylab("Count") + xlab("Year of Diagnosis") + ggtitle("Incidences by Year of Diagnosis") + coord_flip()
```

```{r}
incidenceRace2 <- leukemiaIncidenceDesc %>% filter(!is.na(Count))  %>% filter(Race != "All") %>% dplyr::select(Race, Count) %>% group_by(Race) %>% count(Race, wt=Count) %>% rename("Count" = "n"); incidenceRace2

n <- sum(incidenceRace2$Count)

incidenceRace2 <- mutate(incidenceRace2, Percent = 100*Count/n); incidenceRace2

ggplot(incidenceRace2, aes(Race)) + geom_bar(aes(weight = Count)) + ylab("Count") + xlab("Race") + ggtitle("Incidence by Race") + coord_flip()
```
