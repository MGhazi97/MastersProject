...
```{r}
deathSex2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% filter(Sex != 'Male and female') %>% dplyr::select(Sex, Count) %>% group_by(Sex) %>% count(Sex, wt=Count) %>% rename("Count" = "n"); deathSex2
...
```
```{r}
deathState2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% dplyr::select(State, Count) %>% group_by(State) %>% count(State, wt=Count) %>%  rename("Count" = "n"); arrange(deathState2, Count)


n <- sum(deathState2$Count)

deathState2 <- mutate(deathState2, Percent = 100*Count/n); arrange(deathState2, Count)
...
```{r}
deathYOD2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% dplyr::select(Year.of.Death, Count) %>% filter(Year.of.Death != "1969-2019") %>% group_by(Year.of.Death) %>% count(Year.of.Death, wt=Count) %>% rename("Count" = "n"); deathYOD2
...
```{r}
deathRace2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% filter(Race != 'All') %>% dplyr::select(Race, Count) %>% group_by(Race) %>% count(Race, wt=Count) %>% rename("Count" = "n"); arrange(deathRace2, Count)
