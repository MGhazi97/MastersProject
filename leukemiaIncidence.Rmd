...
```{r}
incidenceSex2 <- leukemiaIncidenceDesc %>% filter(!is.na(Count)) %>% filter(Sex != 'Male and female') %>% dplyr::select(Sex, Count) %>% group_by(Sex) %>% count(Sex, wt=Count) %>% rename("Count" = "n"); incidenceSex2
...
```{r}
incidenceRace2 <- leukemiaIncidenceDesc %>% filter(!is.na(Count)) %>% filter(Race != 'All') %>% filter(Race != "All") %>% dplyr::select(Race, Count) %>% group_by(Race) %>% count(Race, wt=Count) %>% rename("Count" = "n"); arrange(incidenceRace2, Count)
