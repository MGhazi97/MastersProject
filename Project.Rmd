...
#```{r}
leukemiaMortality2 <- read.csv("leukemiaMortality2.csv", header = TRUE); 
leukemiaMortality2 <- leukemiaMortality2 %>% 
   ...
   mutate(Year.of.Death = ifelse(Year.of.Death == 0, "1969-2019", 
                        ifelse(Year.of.Death == 1, "1969-1971", 
                        ifelse(Year.of.Death == 2, "1972-1974",
                        ifelse(Year.of.Death == 3, "1975-1977",
                        ifelse(Year.of.Death == 4, "1978-1980", 
                        ifelse(Year.of.Death == 5, "1981-1983",
                        ifelse(Year.of.Death == 6, "1984-1986",
                        ifelse(Year.of.Death == 7, "1987-1989", 
                        ifelse(Year.of.Death == 8, "1990-1992",
                        ifelse(Year.of.Death == 9, "1993-1995",
                        ifelse(Year.of.Death == 10, "1996-1998", 
                        ifelse(Year.of.Death == 11, "1999-2001",
                        ifelse(Year.of.Death == 12, "2002-2004",  
                        ifelse(Year.of.Death == 13, "2005-2007", 
                        ifelse(Year.of.Death == 14, "2008-2010",
                        ifelse(Year.of.Death == 15, "2011-2015", "2016-2020"))))))))))))))))) %>%
   mutate(Sex = ifelse(Sex == 0, "Male and female", 
                        ifelse(Sex == 1, "Male", "Female"))) %>%
   mutate(State = ifelse(State == 2, "Arizona", 
                        ifelse(State == 4, "California", 
                        ifelse(State == 6, "Connecticut",
                        ifelse(State == 10, "Georgia",
                        ifelse(State == 13, "Illinois", 
                        ifelse(State == 15, "Iowa",
                        ifelse(State == 17, "Kentucky",
                        ifelse(State == 18, "Louisiana", 
                        ifelse(State == 21, "Massachusetts",
                        ifelse(State == 30, "New Jersey",
                        ifelse(State == 31, "New Mexico", 
                        ifelse(State == 32, "New York",
                        ifelse(State == 43, "Texas",  
                        ifelse(State == 44, "Utah", "Washington")))))))))))))))

#leukemiaMortality2

#write.csv(leukemiaMortality2, "leukemiaMortalityDescription.csv", row.names=FALSE)
#```
...
deathAges2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% dplyr::select(Age.Group, Count) %>% group_by(Age.Group) %>% count(Age.Group, wt=Count) %>% rename("Count" = "n"); deathAges2

n <- sum(deathAges2$Count)

deathAges2 <- mutate(deathAges2, Percent = 100*Count/n); deathAges2

ggplot(deathAges2, aes(x = Age.Group, y = Count)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Count), hjust = -0.5)  + coord_flip() +
  scale_y_continuous(labels = function(x) format(x, scientific=FALSE)) +
  ylab("Count") +
...
```{r}
deathSex2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% dplyr::select(Sex, Count) %>% group_by(Sex) %>% count(Sex, wt=Count) %>% rename("Count" = "n"); deathSex2

n <- sum(deathSex2$Count)

deathSex2 <- mutate(deathSex2, Percent = 100*Count/n); deathSex2

ggplot(deathSex2, aes(Sex)) + geom_bar(aes(weight = Count)) + ylab("Count") + xlab("Sex") + ggtitle("Deaths by Sex Group")
```
```{r}
deathState2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% dplyr::select(State, Count) %>% group_by(State) %>% count(State, wt=Count) %>% rename("Count" = "n"); deathState2

n <- sum(deathState2$Count)

deathState2 <- mutate(deathState2, Percent = 100*Count/n); deathState2

ggplot(deathState2, aes(State)) + geom_bar(aes(weight = Count)) + ylab("Count") + xlab("State") + ggtitle("Deaths by State") + coord_flip()
```

```{r}
deathYOD2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% dplyr::select(Year.of.Death, Count) %>% group_by(Year.of.Death) %>% count(Year.of.Death, wt=Count) %>% rename("Count" = "n"); deathYOD2

n <- sum(deathYOD2$Count)

deathYOD2 <- mutate(deathYOD2, Percent = 100*Count/n); deathYOD2

ggplot(deathYOD2, aes(Year.of.Death)) + geom_bar(aes(weight = Count)) + ylab("Count") + xlab("Year of Death") + ggtitle("Deaths by Year of Death") + coord_flip()
```

```{r}
deathRace2 <- leukemiaMortalityDesc %>% filter(!is.na(Count)) %>% dplyr::select(Race, Count) %>% group_by(Race) %>% count(Race, wt=Count) %>% rename("Count" = "n"); deathRace2

n <- sum(deathRace2$Count)

deathRace2 <- mutate(deathRace2, Percent = 100*Count/n); deathRace2

ggplot(deathRace2, aes(Race)) + geom_bar(aes(weight = Count)) + ylab("Count") + xlab("Race") + ggtitle("Deaths by Race") + coord_flip()
```
